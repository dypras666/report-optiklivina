...
export async function fetchProductTransactionsPage({ jenis, productId, cabangId, page = 1, limit = 20, pembukuanId, all, split = false, page_mkt, limit_mkt, page_toko, limit_toko }){
    ...
    sqlMarketing = `
      SELECT t.id_transaksi AS sort_id,
        ...
        SELECT COALESCE(t2id.id_transaksi, t2kode.id_transaksi) AS sort_id,
               ...
               tl.harga_produk,
               tl.jumlah_harga,
            COALESCE(tl.tanggal_log, t2id.tanggal_order, t2kode.tanggal_order, g2.tanggal_grosir, g2.jam_log) AS tanggal_log,
    ...
    sqlMarketing = `
      SELECT t.id_transaksi AS sort_id,
        ...
        SELECT COALESCE(t2id.id_transaksi, t2kode.id_transaksi) AS sort_id,
               ...
               tl2.harga_produk,
               tl2.jumlah_harga,
            COALESCE(tl2.tanggal_log, t2id.tanggal_order, t2kode.tanggal_order, g2.tanggal_grosir, g2.jam_log) AS tanggal_log,
  ...
  const sqlToko = `
    SELECT gl.id_sg_log AS sort_id,
           ...
           gl.harga_produk, gl.jumlah_harga, COALESCE(gl.tanggal_log, g.tanggal_grosir, g.jam_log) AS tanggal_log,
           (CASE 
