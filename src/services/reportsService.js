...
export async function fetchMarketingReports({ pembukuanId, marketingId, cabangId, page = 1, limit = 20, includeSums = false, q, status }){
  ...
  const baseJoin = `
    FROM transaksi
    LEFT JOIN (
      SELECT kode_transaksi, SUM(jumlah_bayar) AS jml_bayar,
  ...
  const sqlData = `
    SELECT transaksi.id_transaksi, transaksi.kode_transaksi, transaksi.tanggal_order,
           customer.nama_customer, cabang_toko.nama_cabang, admin.nama_lengkap,
           ...
           ((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)) AS sisa_bayar,
           ...
           (CASE WHEN ((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)) <= 0 
           ...
           (CASE WHEN IFNULL(tp.jml_bayar,0) > 0 AND (CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) > 0
                 THEN (IFNULL(tp.jml_bayar,0) / (CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END)) 
  ...
  const sqlSum = `
    SELECT SUM((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END)) AS sum_fix_harga,
           ...
           SUM((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)) AS sum_sisa_bayar,
           ...
           SUM(CASE WHEN ((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)) <= 0 
           ...
           SUM(CASE WHEN IFNULL(tp.jml_bayar,0) > 0 AND (CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) > 0
                     THEN (IFNULL(tp.jml_bayar,0) / (CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END))
                          * COALESCE((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - (IFNULL(it.sum_modal,0) + IFNULL(it.sum_ongkir,0)), 0)
...
export async function fetchMarketingReportsByPaymentPeriod({ pembukuanId, marketingId, cabangId, page = 1, limit = 20, includeSums = false, q, status }){
  ...
  if (status === 'lunas') { whereTransaksi += ' AND ((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)) <= 0' }
  if (status === 'belum') { whereTransaksi += ' AND ((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)) > 0' }
  whereTransaksi += ' AND IFNULL(tp.jml_bayar,0) > 0'
...
export async function fetchProfitLossDetails({ pembukuanId, cabangId }){
  ...
  `
  const sql = `
    SELECT s.nama_produk, s.fix_harga, s.jml_bayar,
           (s.fix_harga - s.jml_bayar) AS sisa_bayar,
           s.modal_total,
           (CASE WHEN (s.fix_harga - s.jml_bayar) <= 0 THEN (s.item_amount - (s.modal_total + s.ongkir_total)) ELSE 0 END) AS laba_aktual,
           (CASE WHEN (s.fix_harga - s.jml_bayar) > 0 
                 THEN ((IFNULL(s.jml_bayar,0) + 0.0) / s.fix_harga) * (s.item_amount - (s.modal_total + s.ongkir_total))
...
export async function fetchTokoReports({ pembukuanId, cabangId, page = 1, limit = 50 }){
  ...
  const sqlData = `
    SELECT cabang_toko.id_cabang, cabang_toko.nama_cabang,
           ...
           SUM(CASE WHEN ((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)) <= 0 AND UPPER(tp.jenis_transaksi) = 'GL'
           ...
           SUM(CASE 
                 WHEN ((CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)) > 0
                  AND TIMESTAMPDIFF(MONTH, COALESCE(tp.tanggal_bayar, transaksi.tanggal_order), CURDATE()) <= 1
                 THEN (CASE WHEN (transaksi.harga_nego > 0) THEN transaksi.harga_nego ELSE transaksi.total_harga END) - IFNULL(tp.jml_bayar,0)
...
export async function fetchTokoTransaksiReports({ pembukuanId, cabangId, page = 1, limit = 50, q, status }){
  ...
  const sql = `
    ...
    FROM grosir g
      ...
      LEFT JOIN (
        SELECT id_cabang, kode_grosir, SUM(jumlah_bayar) AS jml_bayar
...
export async function fetchCabangAnalysisAll({ pembukuanId }){
  ...
  for(const id of ids){
    ...
    result.push({ id_cabang: id, nama_cabang: nama, omset_marketing, omset_toko, laba_marketing, laba_toko, persen_bayar_marketing,
...
export async function fetchCustomerProfileByCode({ kode }){
  ...
  const p = rows?.[0] || null
  if (!p) return null
  const ktpPath = p.file_ktp ? `uploads/customer_ktp/${p.file_ktp}` : null
  const kkPath = p.file_kk ? `uploads/customer_kk/${p.file_kk}` : null
  const urlFor = async (rel) => rel ? await buildS3Url({ bucket, key: `${prefix}/${rel}` }) : null
  return {
    ...p,
    file_ktp_url: await urlFor(ktpPath),
    file_kk_url: await urlFor(kkPath)
  }
}

export async function fetchCustomers({ page = 1, limit = 20, cabangId, marketingId, ktp, kk, aging, doc, q, addr }){
  const offset = Math.max(0, (Number(page) - 1) * Number(limit))
  ...
  if (q){
    ...
    where.push('(LOWER(COALESCE(c.nama_customer,\'\')) LIKE ? OR LOWER(COALESCE(c.no_hp,\'\')) LIKE ?)')
  ...
  const sqlBase = `
    FROM customer c
      ...
      LEFT JOIN (
        SELECT kode_transaksi, SUM(jumlah_bayar) AS jml_bayar, MAX(tanggal_bayar) AS tanggal_bayar
        FROM transaksi_pembayaran
        GROUP BY kode_transaksi
      ) tp ON tp.kode_transaksi = t.kode_transaksi
      LEFT JOIN cabang_toko ON cabang_toko.id_cabang = c.cabang
  ...
  const sqlData = `
    SELECT c.kode_customer, c.nama_customer, c.no_hp,
           ...
           c.file_ktp, c.file_kk,
           SUM(IFNULL((CASE WHEN (t.harga_nego > 0) THEN t.harga_nego ELSE t.total_harga END) - IFNULL(tp.jml_bayar,0),0)) AS sisa_total,
           MAX(CASE WHEN IFNULL((CASE WHEN (t.harga_nego > 0) THEN t.harga_nego ELSE t.total_harga END) - IFNULL(tp.jml_bayar,0),0) > 0
                    THEN TIMESTAMPDIFF(MONTH, COALESCE(tp.tanggal_bayar, t.tanggal_order), CURDATE()) ELSE 0 END) AS aging_months,
           MAX(COALESCE(tp.tanggal_bayar, t.tanggal_order)) AS last_activity,
           MIN(t.tanggal_order) AS tanggal_register
    ${sqlBase}
    GROUP BY c.kode_customer, c.nama_customer, c.no_hp, cabang_toko.nama_cabang, admin.nama_lengkap, c.file_ktp, c.file_kk
  ...
  const bucket = process.env.AWS_BUCKET || 'optiklivina'
  const prefix = (process.env.AWS_S3_PREFIX || 'demo').replace(/\/$/, '')
  const build = async (f) => f ? await buildS3Url({ bucket, key: `${prefix}/uploads/customer_ktp/${f}` }) : null
  const build2 = async (f) => f ? await buildS3Url({ bucket, key: `${prefix}/uploads/customer_kk/${f}` }) : null
  const mapped = await Promise.all(rows.map(async r => ({
    ...r,
    file_ktp_url: await build(r.file_ktp),
    file_kk_url: await build2(r.file_kk)
  })))
...
export async function fetchCustomerSummaryByCode({ kode, limit = 50 }){
  ...
  const [transRows] = await pool.query(
    `SELECT t.id_transaksi,
            ...
            ((CASE WHEN (t.harga_nego > 0) THEN t.harga_nego ELSE t.total_harga END) - IFNULL(tp.total_bayar, 0)) AS sisa,
            t.tanggal_order AS tanggal,
            CASE
              WHEN t.jenis_beli = 'kacamata' AND IFNULL(tp.total_bayar, 0) > 0 THEN (
     ...
     FROM transaksi t
       LEFT JOIN (
         SELECT kode_transaksi AS kode_transaksi_bayar, SUM(jumlah_bayar) AS total_bayar
         FROM transaksi_pembayaran
         GROUP BY kode_transaksi
  ...
  const [usedRows] = await pool.query(
    `SELECT id, kode_transaksi, point_use, tanggal_penggunaan AS tanggal
     FROM bayar_point
     WHERE kode_customer = ?
     ORDER BY id DESC
     LIMIT 100`,
    ...
    SELECT t.id_marketing,
           ...
           ,COUNT(DISTINCT CASE 
                 WHEN LOWER(customer.status_user) = 'blacklist'
                  AND ((CASE WHEN (t.harga_nego > 0) THEN t.harga_nego ELSE t.total_harga END) - IFNULL(tp2.jml_bayar,0)) > 0
  ...
  const sqlUnion = `
    ...
    FROM produk p
      ...
      LEFT JOIN (
        ...
        FROM transaksi_log WHERE jenis_produk='katalog' AND id_cabang=?
        GROUP BY id_produk, id_cabang
      ) tl ON tl.id_produk=p.id_produk AND tl.id_cabang=sc.id_cabang
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM grosir_log WHERE jenis_produk='katalog' AND id_cabang=?
        GROUP BY id_produk, id_cabang
    ...
    FROM softlens s
      ...
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM transaksi_log WHERE jenis_produk='softlens' AND id_cabang=?
        GROUP BY id_produk, id_cabang
      ) tl ON tl.id_produk=s.id_softlens AND tl.id_cabang=ssc.id_cabang
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM grosir_log WHERE jenis_produk='softlens' AND id_cabang=?
        GROUP BY id_produk, id_cabang
    ...
    FROM frame f
      ...
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM transaksi_log WHERE jenis_produk='frame' AND id_cabang=?
        GROUP BY id_produk, id_cabang
      ) tl ON tl.id_produk=f.id_frame AND tl.id_cabang=fsc.id_cabang
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM grosir_log WHERE jenis_produk='frame' AND id_cabang=?
        GROUP BY id_produk, id_cabang
    ...
    FROM lensa l
      ...
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM transaksi_log WHERE jenis_produk='lensa' AND id_cabang=?
        GROUP BY id_produk, id_cabang
      ) tl ON tl.id_produk=l.id_lensa AND tl.id_cabang=lsc.id_cabang
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM grosir_log WHERE jenis_produk='lensa' AND id_cabang=?
        GROUP BY id_produk, id_cabang
...
export async function fetchAsetReportAllCabang({ includeIdle = false } = {}){
  const now = Date.now()
  ...
  if(!includeIdle){
    ...
    `
    const t0 = Date.now()
    const [rows] = await pool.query({ sql: sqlSummaryUnion, timeout: 20000 })
    const dur = Date.now() - t0
  ...
  const sqlUnion = `
    SELECT 'katalog' AS jenis,
           ...
           p.harga_modal, p.harga_jual, sc.stok AS stok,
           GREATEST(
             COALESCE((SELECT MAX(tanggal_log) FROM transaksi_log tl WHERE tl.jenis_produk='katalog' AND tl.id_produk=p.id_produk AND tl.id_cabang=sc.id_cabang), '1970-01-01'),
             COALESCE((SELECT MAX(tanggal_log) FROM grosir_log gl WHERE gl.jenis_produk='katalog' AND gl.id_produk=p.id_produk AND gl.id_cabang=sc.id_cabang), '1970-01-01')
           ) AS last_log
    FROM produk p
    ...
    SELECT 'softlens' AS jenis,
           ...
           s.harga_modal, s.harga_jual, ssc.stok AS stok,
           GREATEST(
             COALESCE((SELECT MAX(tanggal_log) FROM transaksi_log tl WHERE tl.jenis_produk='softlens' AND tl.id_produk=s.id_softlens AND tl.id_cabang=ssc.id_cabang), '1970-01-01'),
             COALESCE((SELECT MAX(tanggal_log) FROM grosir_log gl WHERE gl.jenis_produk='softlens' AND gl.id_produk=s.id_softlens AND gl.id_cabang=ssc.id_cabang), '1970-01-01')
           ) AS last_log
    FROM softlens s
    ...
    SELECT 'frame' AS jenis,
           ...
           f.harga_modal, f.harga_jual, fsc.stok_cb AS stok,
           GREATEST(
             COALESCE((SELECT MAX(tanggal_log) FROM transaksi_log tl WHERE tl.jenis_produk='frame' AND tl.id_produk=f.id_frame AND tl.id_cabang=fsc.id_cabang), '1970-01-01'),
             COALESCE((SELECT MAX(tanggal_log) FROM grosir_log gl WHERE gl.jenis_produk='frame' AND gl.id_produk=f.id_frame AND gl.id_cabang=fsc.id_cabang), '1970-01-01')
           ) AS last_log
    FROM frame f
    ...
    SELECT 'lensa' AS jenis,
           ...
           l.harga_modal, l.harga_jual, lsc.stok_masuk AS stok,
           GREATEST(
             COALESCE((SELECT MAX(tanggal_log) FROM transaksi_log tl WHERE tl.jenis_produk='lensa' AND tl.id_produk=l.id_lensa AND tl.id_cabang=lsc.id_cabang), '1970-01-01'),
             COALESCE((SELECT MAX(tanggal_log) FROM grosir_log gl WHERE gl.jenis_produk='lensa' AND gl.id_produk=l.id_lensa AND gl.id_cabang=lsc.id_cabang), '1970-01-01')
           ) AS last_log
    FROM lensa l
...
export async function fetchAsetIdle({ cabangId, months = 3 }){
  ...
  const sqlUnion = `
    SELECT 'katalog' AS jenis, p.id_produk AS id, p.nama_produk AS nama, p.sku_katalog AS sku,
           p.harga_modal, p.harga_jual, sc.stok AS stok,
           GREATEST(
             COALESCE((SELECT MAX(tanggal_log) FROM transaksi_log tl WHERE tl.jenis_produk='katalog' AND tl.id_produk=p.id_produk AND tl.id_cabang=?), '1970-01-01'),
             COALESCE((SELECT MAX(tanggal_log) FROM grosir_log gl WHERE gl.jenis_produk='katalog' AND gl.id_produk=p.id_produk AND gl.id_cabang=?), '1970-01-01')
           ) AS last_log
    FROM produk p
      JOIN produk_stok_cabang sc ON sc.id_produk=p.id_produk AND sc.id_cabang=?
    WHERE sc.stok > 0
    ...
    SELECT 'softlens' AS jenis, s.id_softlens AS id, s.nama_softlens AS nama, s.sku_softlens AS sku,
           s.harga_modal, s.harga_jual, ssc.stok AS stok,
           GREATEST(
             COALESCE((SELECT MAX(tanggal_log) FROM transaksi_log tl WHERE tl.jenis_produk='softlens' AND tl.id_produk=s.id_softlens AND tl.id_cabang=?), '1970-01-01'),
             COALESCE((SELECT MAX(tanggal_log) FROM grosir_log gl WHERE gl.jenis_produk='softlens' AND gl.id_produk=s.id_softlens AND gl.id_cabang=?), '1970-01-01')
           ) AS last_log
    FROM softlens s
      JOIN softlens_stok_cabang ssc ON ssc.id_softlens=s.id_softlens AND ssc.id_cabang=?
    WHERE ssc.stok > 0
    ...
    SELECT 'frame' AS jenis, f.id_frame AS id, (SELECT nama_frame FROM frame_kat WHERE frame_kat.id_kat_frame=f.id_kat_frame) AS nama, f.sku_frame AS sku,
           f.harga_modal, f.harga_jual, fsc.stok_cb AS stok,
           GREATEST(
             COALESCE((SELECT MAX(tanggal_log) FROM transaksi_log tl WHERE tl.jenis_produk='frame' AND tl.id_produk=f.id_frame AND tl.id_cabang=?), '1970-01-01'),
             COALESCE((SELECT MAX(tanggal_log) FROM grosir_log gl WHERE gl.jenis_produk='frame' AND gl.id_produk=f.id_frame AND gl.id_cabang=?), '1970-01-01')
           ) AS last_log
    FROM frame f
      JOIN frame_stok_cabang fsc ON fsc.id_frame=f.id_frame AND fsc.id_cabang=?
    WHERE fsc.stok_cb > 0
    ...
    SELECT 'lensa' AS jenis, l.id_lensa AS id, CONCAT((SELECT nama_lensa_kat FROM lensa_kat WHERE lensa_kat.id_lensa_kat=l.id_lensa_kat),' ', l.size) AS nama, l.sku_lensa AS sku,
           l.harga_modal, l.harga_jual, lsc.stok_masuk AS stok,
           GREATEST(
             COALESCE((SELECT MAX(tanggal_log) FROM transaksi_log tl WHERE tl.jenis_produk='lensa' AND tl.id_produk=l.id_lensa AND tl.id_cabang=?), '1970-01-01'),
             COALESCE((SELECT MAX(tanggal_log) FROM grosir_log gl WHERE gl.jenis_produk='lensa' AND gl.id_produk=l.id_lensa AND gl.id_cabang=?), '1970-01-01')
           ) AS last_log
    FROM lensa l
      JOIN lensa_stok_cabang lsc ON lsc.id_lensa=l.id_lensa AND lsc.id_cabang=?
    WHERE lsc.stok_masuk > 0
  ...
  `
  const t0 = Date.now()
  const [rows] = await pool.query({ sql, values: params, timeout: 30000 })
  const dur = Date.now() - t0
...
export async function fetchMasterItems({ jenis, page = 1, limit = 20, q, status, order, soldOnly }){
  const j = String(jenis||'').toLowerCase()
  const allowed = new Set(['katalog','softlens','frame','lensa'])
  if(!allowed.has(j)) throw new Error('jenis invalid')
  const offset = Math.max(0, (Number(page) - 1) * Number(limit))
  ...
  if(j === 'katalog'){
    ...
    sql = `SELECT 'katalog' AS jenis, p.id_produk AS id, p.nama_produk AS nama, p.sku_katalog AS sku, p.harga_modal, p.harga_jual, COALESCE(ast.status,1) AS status, COALESCE(agg.sum_qty,0) AS total_qty, COALESCE(agg.sum_uang,0) AS total_uang FROM produk p LEFT JOIN asset_status ast ON ast.jenis='katalog' AND ast.product_id=p.id_produk LEFT JOIN (SELECT id_produk, SUM(sum_qty) AS sum_qty, SUM(sum_uang) AS sum_uang FROM (SELECT tb.id_produk, tb.qty AS sum_qty, ((SELECT harga_jual FROM produk WHERE produk.id_produk=tb.id_produk) * tb.qty) AS sum_uang FROM transaksi_barang tb LEFT JOIN transaksi t ON t.kode_transaksi=tb.kode_transaksi UNION ALL SELECT id_produk, jumlah AS sum_qty, jumlah_harga AS sum_uang FROM grosir_log WHERE jenis_produk='katalog') u GROUP BY id_produk) agg ON agg.id_produk=p.id_produk ${whereSqlBasic}${soldOnly ? (whereSqlBasic ? ' AND ' : ' WHERE ') + 'COALESCE(agg.sum_qty,0) > 0' : ''} ${orderSql} ${limit>0? 'LIMIT ? OFFSET ?' : ''}`
    if(soldOnly){
      sqlCount = `SELECT COUNT(*) AS cnt FROM produk p LEFT JOIN asset_status ast ON ast.jenis='katalog' AND ast.product_id=p.id_produk LEFT JOIN (SELECT id_produk, SUM(sum_qty) AS sum_qty FROM (SELECT tb.id_produk, tb.qty AS sum_qty FROM transaksi_barang tb LEFT JOIN transaksi t ON t.kode_transaksi=tb.kode_transaksi UNION ALL SELECT id_produk, jumlah AS sum_qty FROM grosir_log WHERE jenis_produk='katalog') u GROUP BY id_produk) agg ON agg.id_produk=p.id_produk ${whereSqlBasic}${soldOnly ? (whereSqlBasic ? ' AND ' : ' WHERE ') + 'COALESCE(agg.sum_qty,0) > 0' : ''}`
    }else{
    ...
    countParams = soldOnly ? params.slice() : (params.slice())
  }else if(j === 'softlens'){
    ...
    sql = `SELECT 'softlens' AS jenis, s.id_softlens AS id, s.nama_softlens AS nama, s.sku_softlens AS sku, s.harga_modal, s.harga_jual, COALESCE(ast.status,1) AS status, COALESCE(agg.sum_qty,0) AS total_qty, COALESCE(agg.sum_uang,0) AS total_uang FROM softlens s LEFT JOIN asset_status ast ON ast.jenis='softlens' AND ast.product_id=s.id_softlens LEFT JOIN (SELECT id_produk, SUM(sum_qty) AS sum_qty, SUM(sum_uang) AS sum_uang FROM (SELECT ts.id_produk, ts.qty AS sum_qty, ((SELECT harga_jual FROM softlens WHERE softlens.id_softlens=ts.id_produk) * ts.qty) AS sum_uang FROM transaksi_barang ts LEFT JOIN transaksi t ON t.kode_transaksi=ts.kode_transaksi UNION ALL SELECT id_produk, jumlah AS sum_qty, jumlah_harga AS sum_uang FROM grosir_log WHERE jenis_produk='softlens') u GROUP BY id_produk) agg ON agg.id_produk=s.id_softlens ${whereSqlBasic}${soldOnly ? (whereSqlBasic ? ' AND ' : ' WHERE ') + 'COALESCE(agg.sum_qty,0) > 0' : ''} ${orderSql} ${limit>0? 'LIMIT ? OFFSET ?' : ''}`
    if(soldOnly){
      sqlCount = `SELECT COUNT(*) AS cnt FROM softlens s LEFT JOIN asset_status ast ON ast.jenis='softlens' AND ast.product_id=s.id_softlens LEFT JOIN (SELECT id_produk, SUM(sum_qty) AS sum_qty FROM (SELECT ts.id_produk, ts.qty AS sum_qty FROM transaksi_barang ts LEFT JOIN transaksi t ON t.kode_transaksi=ts.kode_transaksi UNION ALL SELECT id_produk, jumlah AS sum_qty FROM grosir_log WHERE jenis_produk='softlens') u GROUP BY id_produk) agg ON agg.id_produk=s.id_softlens ${whereSqlBasic}${soldOnly ? (whereSqlBasic ? ' AND ' : ' WHERE ') + 'COALESCE(agg.sum_qty,0) > 0' : ''}`
    }else{
    ...
    sql = `SELECT 'frame' AS jenis, f.id_frame AS id, (SELECT nama_frame FROM frame_kat WHERE frame_kat.id_kat_frame=f.id_kat_frame) AS nama, f.sku_frame AS sku, f.harga_modal, f.harga_jual, COALESCE(ast.status,1) AS status, COALESCE(agg.sum_qty,0) AS total_qty, COALESCE(agg.sum_uang,0) AS total_uang FROM frame f LEFT JOIN asset_status ast ON ast.jenis='frame' AND ast.product_id=f.id_frame LEFT JOIN (SELECT id_produk, SUM(sum_qty) AS sum_qty, SUM(sum_uang) AS sum_uang FROM (SELECT tf.id_frame AS id_produk, tf.qty AS sum_qty, ((SELECT harga_jual FROM frame WHERE frame.id_frame=tf.id_frame) * tf.qty) AS sum_uang FROM transaksi_frame tf LEFT JOIN transaksi t ON t.kode_transaksi=tf.kode_transaksi UNION ALL SELECT id_produk, jumlah AS sum_qty, jumlah_harga AS sum_uang FROM grosir_log WHERE jenis_produk='frame') u GROUP BY id_produk) agg ON agg.id_produk=f.id_frame ${whereSqlBasic}${soldOnly ? (whereSqlBasic ? ' AND ' : ' WHERE ') + 'COALESCE(agg.sum_qty,0) > 0' : ''} ${orderSql} ${limit>0? 'LIMIT ? OFFSET ?' : ''}`
    if(soldOnly){
      sqlCount = `SELECT COUNT(*) AS cnt FROM frame f LEFT JOIN asset_status ast ON ast.jenis='frame' AND ast.product_id=f.id_frame LEFT JOIN (SELECT id_produk, SUM(sum_qty) AS sum_qty FROM (SELECT tf.id_frame AS id_produk, tf.qty AS sum_qty FROM transaksi_frame tf LEFT JOIN transaksi t ON t.kode_transaksi=tf.kode_transaksi UNION ALL SELECT id_produk, jumlah AS sum_qty FROM grosir_log WHERE jenis_produk='frame') u GROUP BY id_produk) agg ON agg.id_produk=f.id_frame ${whereSqlBasic}${soldOnly ? (whereSqlBasic ? ' AND ' : ' WHERE ') + 'COALESCE(agg.sum_qty,0) > 0' : ''}`
    }else{
    ...
    sql = `SELECT 'lensa' AS jenis, l.id_lensa AS id, CONCAT((SELECT nama_lensa_kat FROM lensa_kat WHERE lensa_kat.id_lensa_kat=l.id_lensa_kat), ' ', l.size) AS nama, l.sku_lensa AS sku, l.harga_modal, l.harga_jual, COALESCE(ast.status,1) AS status, COALESCE(agg.sum_qty,0) AS total_qty, COALESCE(agg.sum_uang,0) AS total_uang FROM lensa l LEFT JOIN asset_status ast ON ast.jenis='lensa' AND ast.product_id=l.id_lensa LEFT JOIN (SELECT id_produk, SUM(sum_qty) AS sum_qty, SUM(sum_uang) AS sum_uang FROM (SELECT tl.id_lensa AS id_produk, tl.qty AS sum_qty, ((SELECT harga_jual FROM lensa WHERE lensa.id_lensa=tl.id_lensa) * tl.qty) AS sum_uang FROM transaksi_lensa tl LEFT JOIN transaksi t ON t.kode_transaksi=tl.kode_transaksi UNION ALL SELECT id_produk, jumlah AS sum_qty, jumlah_harga AS sum_uang FROM grosir_log WHERE jenis_produk='lensa') u GROUP BY id_produk) agg ON agg.id_produk=l.id_lensa ${whereSqlBasic}${soldOnly ? (whereSqlBasic ? ' AND ' : ' WHERE ') + 'COALESCE(agg.sum_qty,0) > 0' : ''} ${orderSql} ${limit>0? 'LIMIT ? OFFSET ?' : ''}`
    if(soldOnly){
      sqlCount = `SELECT COUNT(*) AS cnt FROM lensa l LEFT JOIN asset_status ast ON ast.jenis='lensa' AND ast.product_id=l.id_lensa LEFT JOIN (SELECT id_produk, SUM(sum_qty) AS sum_qty FROM (SELECT tl.id_lensa AS id_produk, tl.qty AS sum_qty FROM transaksi_lensa tl LEFT JOIN transaksi t ON t.kode_transaksi=tl.kode_transaksi UNION ALL SELECT id_produk, jumlah AS sum_qty FROM grosir_log WHERE jenis_produk='lensa') u GROUP BY id_produk) agg ON agg.id_produk=l.id_lensa ${whereSqlBasic}${soldOnly ? (whereSqlBasic ? ' AND ' : ' WHERE ') + 'COALESCE(agg.sum_qty,0) > 0' : ''}`
    }else{
...
export async function fetchAsetIdleSummaryAll({ months = 3 }){
  ...
  const sqlUnion = `
    ...
    FROM produk p
      ...
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM transaksi_log WHERE jenis_produk='katalog'
        GROUP BY id_produk, id_cabang
      ) tl ON tl.id_produk=p.id_produk AND tl.id_cabang=sc.id_cabang
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM grosir_log WHERE jenis_produk='katalog'
        GROUP BY id_produk, id_cabang
      ) gl ON gl.id_produk=p.id_produk AND gl.id_cabang=sc.id_cabang
    WHERE sc.stok > 0 AND COALESCE(ast.status,1) = 1
    UNION ALL
    ...
    FROM softlens s
      ...
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM grosir_log WHERE jenis_produk='softlens'
        GROUP BY id_produk, id_cabang
      ) gl ON gl.id_produk=s.id_softlens AND gl.id_cabang=ssc.id_cabang
    WHERE ssc.stok > 0 AND COALESCE(ast.status,1) = 1
    UNION ALL
    ...
    FROM frame f
      ...
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM grosir_log WHERE jenis_produk='frame'
        GROUP BY id_produk, id_cabang
      ) gl ON gl.id_produk=f.id_frame AND gl.id_cabang=fsc.id_cabang
    WHERE fsc.stok_cb > 0 AND COALESCE(ast.status,1) = 1
    UNION ALL
    ...
    FROM lensa l
      ...
      LEFT JOIN (
        SELECT id_produk, id_cabang, MAX(tanggal_log) AS last_log
        FROM grosir_log WHERE jenis_produk='lensa'
        GROUP BY id_produk, id_cabang
      ) gl ON gl.id_produk=l.id_lensa AND gl.id_cabang=lsc.id_cabang
    WHERE lsc.stok_masuk > 0 AND COALESCE(ast.status,1) = 1
  `
  ...
  `
  const t0 = Date.now()
  const [rows] = await pool.query({ sql, values: [months], timeout: 30000 })
  const dur = Date.now() - t0
